...
def get_adjusted_threshold(base_threshold: float, mask_probability: float, similarity: float) -> float:
    ...
    # 마스크 가능성이 높으면 더 낮은 임계값 적용
    # 유사도가 0.25~0.30 사이일 때 마스크로 판단하고 0.22~0.25 임계값 사용
    if similarity < 0.28:
        # 매우 낮은 유사도 → 마스크 가능성 매우 높음 → 임계값 많이 낮춤
        adjusted_threshold = base_threshold - 0.08  # 0.30 → 0.22
    elif similarity < 0.32:
        # 낮은 유사도 → 마스크 가능성 높음 → 임계값 낮춤
        adjusted_threshold = base_threshold - 0.05  # 0.30 → 0.25
    else:
        # 중간 유사도 → 마스크 가능성 중간 → 약간 낮춤
        adjusted_threshold = base_threshold - 0.03  # 0.30 → 0.27
    
    # 최소값 보장 (너무 낮으면 오탐 증가, 최소 0.20)
    return max(adjusted_threshold, 0.20)
