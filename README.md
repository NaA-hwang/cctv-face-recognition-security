...
### ✨ 3단계 워크플로우
1. **영상 업로드**: 실제 CCTV 동영상 파일(.mp4, .avi, .mov) 업로드 또는 실시간 웹캠 
2. **용의자 선택**: 4명 팀원 중 모니터링할 용의자 선택
3. **실시간 분석**: AI 기반 동영상 프레임별 얼굴 인식 및 알림
...
## 🎮 사용법
1. **동영상 업로드**: 실제 CCTV 동영상 파일(.mp4, .avi, .mov) 선택하여 업로드
2. **용의자 선택**: 모니터링할 용의자 선택  
3. **프레임 분석**: 동영상 각 프레임별 AI 얼굴 인식 및 실시간 알림 확인

## 🎬 동영상 분석 모드
실제 동영상 파일 처리:
- **지원 형식**: MP4, AVI, MOV, MKV 등 주요 동영상 포맷
- **프레임별 분석**: 초당 프레임 단위로 얼굴 인식 수행
- **실시간 결과**: 용의자 검출 시 즉시 알림 및 위치 표시
- **진행률 표시**: 동영상 분석 진행상황 실시간 업데이트
